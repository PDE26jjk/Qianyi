...
CACHE_KEY = "__QY_Manager_SINGLETON__"
...
class TaskManager:
    ...
    def __new__(cls):
        if not hasattr(sys, CACHE_KEY):
            with cls._lock:
                if not hasattr(sys, CACHE_KEY):
                    console_print("创建TaskManager唯一实例中...")
    ...
    def start(self):
        ...
        self.running = True
        self.thread = threading.Thread(target=self._run, daemon=True, name="TaskManager")
        self.thread.start()
    ...
    def stop(self):
        ...
        if self.thread:
            ...
            if self.thread.is_alive():
                console_print("TaskManager线程未正常停止")
            self.thread = None
        ...
        console_print("TaskManager管理线程已停止")

    def _run(self):
        console_print("TaskManager管理线程开始运行")
    ...
    def _cleanup(self):
        ...
        with self.action_lock:
            for action_id, action in self.pending_actions.items():
                action._set_exception(RuntimeError("TaskManager已停止"))
    ...
    def init(self) -> Action:
        ...
        # 提交初始化命令
        self._submit_command("INIT", None, action_id)
