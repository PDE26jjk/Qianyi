...
- [x] 1.1 Create the library package layout (curve primitives, panel/edge specification, component protocol, helpers) and verify it imports in a plain Python session that has only numpy installed
...
- [x] 1.3 Implement the panel and edge specification, including the closed counter-clockwise loop validation, automatic positional names for unlabelled edges, rejection of duplicate labels, and the sewability flag; verify with unit tests for each rule
...
- [x] 1.6 Write two example components (a square panel with width/height, and a waistband) and verify their unit tests cover area, closure and label assignment
- [x] 1.7 Add the local-frame stability check and verify that changing a length or width parameter keeps the anchoring region of a panel within the configured tolerance of its previous position

## 2. Generator model and rebuild

- [ ] 2.1 Add the generator model (component id, parameter block, slot outputs) plus the single field that links a panel to its generator; verify a project can save and reload a generator with its panels (link logic verified by clearing the in-memory uuid map and re-resolving; the .blend round trip still needs a manual check, see the notes at the end of this file)
- [x] 2.2 Implement generator creation from a component and verify the project receives one panel per slot with the slot's name
- [x] 2.3 Implement the rebuild entry point (build, then write panels by slot, reusing the existing panel per slot) and verify that regenerating does not create duplicate panels
- [x] 2.4 Implement the in-place write path (rewriting existing vertices, handles and spline points when a panel's edge list is unchanged) and verify edge identities are unchanged after a shape-only parameter change
- [x] 2.5 Implement the rebuild fallback for a changed edge list and verify the panel is rebuilt without duplicating slots or losing its placement, fabric and collision settings
- [x] 2.6 Wire parameter changes to the rebuild entry point and verify that editing a parameter updates the panels with no further user action
- [x] 2.7 Report invalid generator state (unknown component, invalid parameters) without touching the existing panels and verify the user sees an error
...
- [x] 2.9 Implement group lifecycle: deleting any generated panel deletes the whole group, and detaching turns the whole group into ordinary panels; verify both with a scripted scene
...
## 3. Locking of generated panels

- [x] 3.1 Audit the 2D editing operators and list every path that can change panel geometry; deliver the list as the input for 3.2 (guarded: add vertex, add spline point, edge mode move, element delete in edge mode, internal line pen, pattern scale; left free: pattern pen and curve import, which create new panels)
- [x] 3.2 Add the guard to the geometry-editing operators (vertex add/move, spline points, edge move, internal lines, scale) and verify each refuses a generated panel with a visible message while leaving geometry unchanged (guards are in place and the lock message was verified headless; the interactive click-through still needs a session with a viewport)
- [x] 3.3 Verify the operations that must stay available on a generated panel: sewing, simulation, fabric and collision settings, grain direction, 2D placement, mirror and instance copies, and deletion (no guard was added to those paths: move, rotate, copy instance, sewing, material and simulation code)
- [x] 3.4 Verify that a detached panel accepts every editing operation again (locking resolves through the generator link, which detach clears, so a detached panel has no lock at all)

## 4. Sewing remap

- [x] 4.1 Implement the pre-rebuild snapshot (edge identity, label, sampled boundary points per output panel) and verify it is taken before any panel is written (taken at the top of the rebuild, for every member of each instance chain)
...
- [x] 4.3 Implement the geometric fallback (nearest boundary point on the rebuilt panel) and verify a rebuild that removes a label still reconnects to the nearest edge (nearest-edge match over the previous edge's sample points; the fallback runs whenever the label is absent)
...
- [x] 4.6 Scope the remap to the panels of the rebuilt generator and verify an unrelated sewing elsewhere in the project is untouched (only edge identities present in the snapshot are considered, so the far side of a sewing on another panel keeps its identity)

## 5. Component hook prototype

- [x] 5.1 Write a component whose own parameter changes its edge list (for example an optional opening on one edge) so the hook has a real caller (the notched panel: turning the notch on replaces the top edge with three edges)
...
- [x] 7.1 Build an end-to-end headless Blender script (generate a multi-panel component, sew it to a hand-drawn panel, run the simulation) and verify the scene simulates without errors (verified by the maintainer in a live session; simulation was run and accepted)
- [x] 7.2 Verify that a shape-only parameter change keeps every sewing on the generated panels, and that a topology-changing parameter change matches or removes them as specified (sewing survival across a shape change is covered by the scripted scene; the maintainer's run confirmed the end-to-end path)
- [ ] 7.3 Export the generated scene through the scene-capture path and verify the engine harness can load and run it

## 8. Validation

- [x] 8.1 Run `openspec validate parametric-panel-generation --strict` and fix any reported issue (valid; the duplicate task id it reported earlier was fixed)
...
| generator: creates panels, parameter change rebuilds, in-place keeps edge identity, delete one deletes the group, detach keeps panels, invalid parameters leave panels untouched, outline refused before the mesh, copies follow their source | covered by the scripted scene |
| generator: a component with more slots adds only new panels | no component with a variable slot count exists yet - open |
| generator: generated panel sewn to a hand-drawn panel | covered by the user's simulation run |
| locking: vertex edit refused, copy of a generated panel refused, detached panel editable | guard verified as a function; the click-through needs a viewport - open |
| locking: sewing allowed | covered (a sewing on a generated panel was built and survived a rebuild) |
...
- [x] 10.1 Generate a thumbnail per component (numpy rasterisation of the panel outlines) and verify a thumbnail is produced for every built-in component
...
- [x] 10.5 Check every panel with the editor's own outline test before writing it, and verify that an impossible parameter (for example a negative height) leaves the panels unchanged and reports an error instead of rebuilding
...
- [x] 11.6 Index the sewings once per rebuild instead of scanning them per panel, and verify the scripted scene still passes
