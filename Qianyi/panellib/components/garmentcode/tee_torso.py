...
from ._kernel import line_segment, segments_to_panel
...
def _half_segments(width, bottom_width, length, shoulder):
    ...
    # [0, length + shoulder] (clockwise); ``segments_to_panel`` reverses it.
...
def build(params: dict):
    return [
        segments_to_panel("torso_front_half", front_half_segments(params), scale=CM_TO_MM,
                          edge_names={1: "outside", 2: "shoulder"}),
        segments_to_panel("torso_back_half", back_half_segments(params), scale=CM_TO_MM,
                          edge_names={1: "outside", 2: "shoulder"}),
    ]
