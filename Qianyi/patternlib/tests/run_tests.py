...
def _load_patternlib():
    """Import the library as a top-level package named ``patternlib``."""
    spec = importlib.util.spec_from_file_location(
        "patternlib", LIB_DIR / "__init__.py",
        submodule_search_locations=[str(LIB_DIR)],
    )
    module = importlib.util.module_from_spec(spec)
    sys.modules["patternlib"] = module
    spec.loader.exec_module(module)
    return module


patternlib = _load_patternlib()

from patternlib import component, registry  # noqa: E402
from patternlib.curves import Arc, Bezier, FrontendEdge, Line, Polyline, signed_area  # noqa: E402
from patternlib.noise import noise_run  # noqa: E402
from patternlib import preview  # noqa: E402
from patternlib.land import land_pattern, landed_boundary  # noqa: E402
from patternlib.spec import (  # noqa: E402
    EdgeSpec,
    PatternSpec,
    PatternSpecError,
    auto_label,
...
class NoiseTests(unittest.TestCase):
    ...
    def test_run_starts_and_ends_on_its_vertices(self):
        ...
        # Points are the requested count, and the run can be handed to a pattern.
...
class PatternSpecTests(unittest.TestCase):
    ...
    def test_unnamed_edges_get_positional_labels(self):
        pattern = finalize_pattern(self._square())
        self.assertEqual([edge.name for edge in pattern.edges],
                         ["hem", auto_label(1), auto_label(2), auto_label(3)])

    def test_duplicate_labels_are_rejected(self):
        with self.assertRaises(PatternSpecError):
            finalize_pattern(self._square(labels=("hem", "hem", "", "")))

    def test_reserved_positional_label_is_rejected(self):
        with self.assertRaises(PatternSpecError):
            finalize_pattern(self._square(labels=("edge1", "", "", "")))

    def test_open_loop_is_rejected(self):
        pattern = self._square()
        pattern.edges[2] = EdgeSpec(2, 0, Line())
        with self.assertRaises(PatternSpecError):
            finalize_pattern(pattern)

    def test_clockwise_pattern_is_rejected(self):
        pattern = self._square()
    ...
    def test_finalize_does_not_mutate_its_input(self):
        pattern = self._square()
        finalize_pattern(pattern)
        self.assertEqual(pattern.edges[1].name, "")
...
class ComponentTests(unittest.TestCase):
    ...
    def test_same_params_give_the_same_result(self):
        ...
        self.assertEqual([p.name for p in first.patterns], [p.name for p in second.patterns])
    ...
    def test_shape_change_keeps_the_reference_point(self):
        ...
        shift = reference_shift(base.patterns[0], changed.patterns[0])
    ...
    def test_waistband_is_two_bands_with_a_waist_edge(self):
        ...
        self.assertEqual([p.name for p in spec.patterns], ["band_front", "band_back"])
...
class LandingTests(unittest.TestCase):
    ...
    def test_landed_pattern_becomes_editor_edges(self):
        spec = component.build_component("waistband", {"waist_length": 800.0,
                                                       "waist_curve": 20.0})
        landed = land_pattern(spec.patterns[0])
    ...
    def test_boundary_is_one_closed_loop(self):
        ...
        points = landed_boundary(land_pattern(spec.patterns[0]))
