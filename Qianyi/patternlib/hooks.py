...
class RebuildContext:
    ...
    def old_edge(self, pattern: str, name: str = "", index: int = -1) -> EdgeRef | None:
        return _find(self.old_edges, pattern, name, index)

    def new_edges_of(self, pattern: str, name: str = "") -> list[EdgeRef]:
        return [edge for edge in self.new_edges
                if edge.pattern == pattern and (not name or edge.name == name)]
...
def _find(edges: list[EdgeRef], pattern: str, name: str, index: int) -> EdgeRef | None:
    for edge in edges:
