"""Curve primitives for parametric panels.

A curve owns shape, not endpoints: the panel's vertex list owns the two end
points and every primitive is evaluated between them. That keeps a panel's
topology (which vertices an edge connects) separate from its geometry, so a
shape change never renumbers the panel.
...
@dataclass(frozen=True)
class Line(Curve):
    """Straight edge between the panel vertices."""
...
class Bezier(Curve):
    ...
    Control points are absolute panel coordinates. A quadratic curve is raised
...
@dataclass(frozen=True)
class Arc(Curve):
    """Circular arc between the two panel vertices.
...
class Polyline(Curve):
    ...
    The first and last points must coincide with the panel vertices the edge
