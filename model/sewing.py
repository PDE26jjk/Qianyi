...
from ..gizmos import sewing_renderer
...
class SewingOneSide(PropertyGroup):
    ...
    def update_data(self, line1, pos1, line2, pos2):
...
class Sewing(PropertyGroup, ModelData):
...
def calc_sewing_side_edges(ss):
    ...
    crazy_loop = e1_i == e2_i and ss.pos1 > ss.pos2

    if crazy_loop:
        e_i = (e_i + 1) % len(p.edges)
...
def calc_sewing_side_sections(ss, sections_start_end, reverse=False):
    ...
    if reverse:
        sections.reverse()
    # edges: List[Edge2D] = calc_sewing_side_edges(ss)
    #
    # sections: List[Section] = []
    # sec1 = edges[0].add_section(ss.pos1)
    # sec2 = edges[-1].add_section(ss.pos2)
    # assert sec1 is not None and sec2 != sec1
    #
    # e = edges[0]
    # start_i = 0
    # while e.sections[start_i] != sec1:
    #     start_i += 1
    # e = edges[-1]
    # end_i = 0
    # if sec2 is not None:
    #     while e.sections[end_i] != sec2:
    #         end_i += 1
    # else:
    #     end_i = len(e.sections)
    #
    # if len(edges) == 1:
    #     sections.extend(e.sections[start_i:end_i])
    # else:
    #     sections.extend(edges[0].sections[start_i:])
    #     for i in range(1, len(edges) - 1):
    #         sections.extend(edges[i].sections)
    #     sections.extend(edges[-1].sections[:end_i])
    # sections = []
    # sec1 = edges[0].add_section(ss.pos1)
    # sec2 = edges[-1].add_section(ss.pos2)

    # print(start_i,end_i,sections)
...
def calc_sewing_geo_point(project):
    ...
    for sewing in project.sewings:
        ss1, ss2 = sewing.side1, sewing.side2
        sections1, lengths1, scans1 = calc_sewing_side_sections(ss1, sewing.sections1)
        sections2, lengths2, scans2 = calc_sewing_side_sections(ss2, sewing.sections2, sewing.reverse)
        i = j = 0
        ...
        while i < n1 and j < n2:
            ...
            if scans1[i] <= scans2[j]:
                ...
                s1, s2 = sections2[j].split(radio, sewing.reverse)
                ...
                sections2[j].link_to(s1, sewing.reverse)
                sections1[i] = s2
