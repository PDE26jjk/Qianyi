...
patterns, and remove only those seams that no longer have a counterpart.
...
on that edge. Generated patterns MUST NOT require a different sewing
representation, so that generated and hand-drawn patterns remain interchangeable.

#### Scenario: Existing sewing data is unchanged

- **WHEN** a sewing is created between a generated pattern and a hand-drawn pattern
- **THEN** it is stored in the same form as a sewing between two hand-drawn patterns

### Requirement: Unchanged patterns keep their sewings untouched

When a rebuild preserves a pattern's edge list, every sewing on that pattern SHALL
keep its edge identities and positions.

#### Scenario: Shape-only parameter change

- **WHEN** a parameter changes and the pattern's edges are updated in place
- **THEN** all sewings on that pattern keep the same edges and positions

### Requirement: Edges are matched when the topology changes

When a rebuild changes a pattern's edge list, the system SHALL match each previous
edge to a new edge of the same pattern, using the edge label first and geometry
second.
...
- **THEN** the system matches it to the new edge nearest to its previous position on the pattern boundary
...
Matching and re-anchoring SHALL consider only the sewings that touch the patterns
...
The system SHALL call a hook after a component's patterns have been rewritten,
...
#### Scenario: Instance copy of a rebuilt pattern

- **WHEN** a rebuilt pattern has instance copies and those copies carry sewings
