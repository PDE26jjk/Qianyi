# agent-pattern-generators Specification

## Purpose
Let a script or an agent build parametric patterns from the component library,
...
Building a component SHALL return the outlines it would produce - pattern names,
...
- **THEN** the scene's patterns, objects and undoes are exactly as they were
...
name, build the component's patterns into the project, and return the generator's
name together with the patterns it produced. A parameter the component does not
...
- **THEN** it owns the component's patterns, their meshes are built, and the result names the generator and the patterns
...
generator once, and SHALL return the report of that rebuild: how many patterns were
...
so a rebuild SHALL NOT throw a pattern's simulation result away when the mesh
stage can carry it, and the rebuild report SHALL say whether positions were
carried over. Patterns whose simulated positions are no longer the rest pose are
the case a caller may want to settle by running a few simulation frames.

#### Scenario: A parameter change after a run

- **WHEN** a generator's parameter changes after its patterns have been simulated
- **THEN** the report says the simulated positions were carried over and the patterns are not back on their rest pose
...
Detaching SHALL turn the generator's patterns into ordinary patterns and remove the
generator; removing SHALL remove the generator together with the patterns it owns
and report what went. Neither SHALL ask for confirmation.

#### Scenario: Detach keeps the patterns

- **WHEN** a generator is detached
- **THEN** its patterns stay, report that they no longer come from a generator, and the generator is gone

#### Scenario: Removing the group

- **WHEN** a generator is removed
- **THEN** its patterns and the sewings that used them are gone and the result reports the counts

### Requirement: Generated patterns stay editable through the surface

A generated pattern SHALL be readable and its geometry SHALL be editable through
the surface - the interactive lock does not apply here - and the pattern record
SHALL say that it comes from a generator, so a caller can expect a later rebuild
to rewrite what it changed.

#### Scenario: Editing a generated pattern

- **WHEN** a generated pattern's geometry is edited through the surface
- **THEN** the edit is applied and the pattern reports that it comes from a generator

### Requirement: Fabric is assigned by name

Creating a pattern or assigning a fabric SHALL take the fabric's name, and SHALL
...
- **WHEN** a pattern is created without a fabric name
