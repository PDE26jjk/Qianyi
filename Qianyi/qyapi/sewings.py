...
def sew_at(pattern_a, edge_a, position_a, pattern_b, edge_b, position_b,
    ...
    sewing = _stitch(project, first, float(position_a), second, float(position_b),
                     color)
    return _created(project, sewing,
                    f"sew {first_pattern.name}[{first_index}] to "
                    f"{second_pattern.name}[{second_index}]")
...
def _stitch(project, first, first_position, second, second_position, color):
    ...
    what it does when the editor clicks them.
    """
    point_a = edge_point_at(first, float(first_position))
    point_b = edge_point_at(second, float(second_position))
    return project.add_sewing1to1_from_points(first, point_a, second, point_b,
                                              color=color)
