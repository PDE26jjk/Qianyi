...
Let a user see which pattern components the add-on offers, narrow that list down,
...
- **THEN** each component appears as a cell with a thumbnail of its patterns and its name

#### Scenario: Thumbnail failure does not break the pattern

- **WHEN** a thumbnail cannot be produced for a component
- **THEN** the cell still shows the component's name and the pattern stays usable
