...
- The pattern editor keeps its selection twice: the `is_selected` flag on each
  ...
  `operators/select.py`.
- A panel's mesh object points back at its panel through
...
- An instance chain is a circular `instance_next_uuid` list, and
  `model/generator.py:instance_chain()` already returns a panel plus its copies.
...
- One panel = one selection, whichever editor it was made in, including the
  instance chain.
...
- Syncing the sub-selection (vertices and edges) or the sewing selection. Only
  whole panels take part; the edge-mode selection has no 3D counterpart.
- Selecting the collider objects from a pattern (a panel and the body are not
  the same thing).
- A general selection framework, or hooks into Blender's own selection system.
- Highlighting a selected panel in the 3D viewport (Blender already outlines the
  selected object).
...
### D3. Exactly the selected panels are mirrored, not their instance chains

The mirror is one panel for one panel. An instance copy is a convenience for
editing the same panel twice - editing already writes to every member of the
...
the colliders and any imported prop keep their selection; a mesh whose panel has
...
ordinary outline and then, for a selected panel, a second pass in the highlight
colour at a wider width. The width is set explicitly on every call, so the
highlight cannot leak into the next panel's lines.

*Alternative:* recolour the single outline for a selected panel. Rejected: on a
`mesh` or `solid` panel a slightly different colour is easy to miss, and the
...
- [The highlight hides the invalid-outline marker] -> the marker is drawn after
  the highlight, so a crossing outline stays visible on a selected panel.
...
- Whether the sewing and edge selections should follow the panel selection into
  3D (today the 3D side has nothing to show for them).
- Whether a panel's mesh should be highlighted in the 3D viewport with the same
