...
- [x] 2.3 Apply a division to every member of the instance chain and verify the
  ...
  (`_chain_members` / `_divide_member`; the check divides one panel of a linked
...
- [x] 2.4 Add the operator and workspace tool for both modes, with live preview
  ...
  one undo step; the maintainer verified the panel and the re-run in a live
...
- [x] 3.1 Implement the corner command with `ROUND`, `CHAMFER` and `CONCAVE`:
  ...
  adding material - the mirrored arc lies between the chord and the panel - so
...
- [x] 3.3 Make a multi-vertex run apply when the tangent lengths do not overlap
  ...
  tangent length along the edges, the arc lands in the notch, and the panel
  ...
  plain millimetre figure: it used to be a Blender length, which the panel read
  ...
  panel's sampling size when that is longer): the outline keeps every vertex and
  ...
  sampler cannot take: on a 100 mm panel a 3 mm piece never came back from
...
- [x] 3.4 Wire the radius to the drag and to the redo panel, and verify a re-run
  ...
  and `BLOCKING`, and `execute` recomputes from the selection, so the panel's
  ...
  not fit - the panel is not a place to report that - and a radius below the
  smallest finishes the run with nothing changed, which is what actually takes
  the panel back to how it was, because a cancelled re-run keeps the last
...
- [x] 4.1 Implement the fan: pivot and target on the outline, radius from the
  ...
  that lies along the outline is refused - it does not divide the panel - which
...
- [x] 4.2 Implement and report the added area, and refuse the cases in the spec
  ...
  crossing itself)
  (checked: a point 50 mm inside the panel is refused by name and distance, a
  chord that leaves a notched panel is refused with the point where it meets the
...
- [x] 4.3 Verify no internal line is created for either radius, that the
  ...
  rotation
  (checked: the panel gains no internal line, every stationary vertex keeps its
...
- [x] 4.4 Wire the angle to the drag and the redo panel, and verify a re-run
  ...
  the panel as it was. Registered with `REGISTER`, `UNDO`, `GRAB_CURSOR` and
...
- [x] 10.4 Make the geometry commands act on the current selection (hover only
  ...
  (`pattern_geometry.selected_edge_run` resolves the panel and the edge indices
...
- [x] 10.6 Recompute the seam remapping inside the command from the
  ...
  command ends by setting the panel's `need_sewing_update` signal - it never
