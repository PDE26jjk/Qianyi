"""Thumbnails for the library panel: a pattern outline drawn into an RGBA buffer."""

from __future__ import annotations

import numpy as np

from .land import LandedPattern, landed_boundary

BACKGROUND = np.array((0.16, 0.16, 0.18, 1.0), dtype=np.float32)
OUTLINE = np.array((0.85, 0.86, 0.90, 1.0), dtype=np.float32)


def render_preview(patterns: list[LandedPattern], size: int = 96) -> np.ndarray:
    """Rasterise the boundaries of ``patterns`` into a ``(size, size, 4)`` image.
    ...
    boundaries = [landed_boundary(pattern, per_edge=24) for pattern in patterns]
    ...
    for points in boundaries:
        ...
        # Flip Y so the pattern is not drawn upside down in the thumbnail.
