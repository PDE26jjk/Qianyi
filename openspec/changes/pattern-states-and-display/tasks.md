...
- [ ] 1.2 Make copies, instance chains and generator rebuilds carry the state
  and the multiplier, and verify a copied pattern inherits its source's state and
  a generator rebuild does not reset it
- [ ] 1.3 Add the per-fabric display color with a neutral default, and verify
  two patterns on different fabrics draw in their own color in `solid`
- [ ] 1.4 Add the state and multiplier to the scene capture, and verify
  `scene.json` records the state of every pattern

## 2. Engine payload mapping

- [ ] 2.1 Skip `excluded` patterns when the bridge builds the mesh list, and
...
- [ ] 2.2 Implement `frozen` as full pin weights in `QYPinFix`, recording the
  pinned vertex set per pattern, and verify unfreeze removes exactly that set and
  leaves hand-made pins intact
- [ ] 2.3 Implement `stiffened` as a per-pattern scale on the payload's stretch
  and bending, and verify the payload carries the scaled values while the
  shared fabric asset keeps its own
- [ ] 2.4 Refuse a simulation start when no pattern participates, and verify the
  report names the reason and the engine is not called
- [ ] 2.5 Stop a running simulation when a participating pattern's state or
  multiplier changes, and verify the change survives and the report says it
  applies from the next start
- [ ] 2.6 Verify a frozen pattern does not move and a simulated pattern drapes
...
- [ ] 3.2 Show and edit the same values in the 3D sidebar of the selected
  pattern, including for patterns the sidebar currently treats as non-pattern
  meshes
...
- [x] 4.2 Implement the display branch the current code marks as a TODO, with
  ...
  sampled mesh edges
- [x] 4.3 Implement `solid` from the fabric display color and verify each pattern
...
- [x] 4.4 Implement `debug` from the engine's per-vertex colors and `stress`
  ...
  produced
- [ ] 4.5 Verify that switching modes never resamples a pattern, rebuilds a mesh
...
- [x] 5.2 Draw the patterns again with a colour per vertex - the strain ramp or
...
- [x] 5.9 Draw the seam preview from the scene (the sewing objects and the
  patterns' current meshes) so it works with no simulation and whatever shape key
...
- [x] 6.2 Draw the projected triangles filled behind the patterns and the
...
- [x] 6.3 Add the project properties (collection, switch, axis, offset, colour,
  fill and mesh opacities) and the pattern editor pattern that exposes them, with
...
- [x] 6.4 Cache the projection on the selection, the transforms, the settings
  ...
  deforms
- [x] 6.5 Verify the guide is display-only: enabling it changes no pattern
...
- [ ] 8.1 Build one scene that uses all four states, every display mode, the
  silhouette and the overlays, step it, and verify: the frozen pattern does not
  move, the excluded pattern is absent from the payload, the stiffened pattern
