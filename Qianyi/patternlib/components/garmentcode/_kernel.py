...
* :class:`Segment` helpers - build a counter-clockwise ``PatternSpec`` from a
...
def segments_to_gcd_panel(name: str, segments, scale: float = 1.0,
                      edge_names: dict | None = None) -> PatternSpec:
    """Turn a closed segment chain into a counter-clockwise ``PatternSpec``.
