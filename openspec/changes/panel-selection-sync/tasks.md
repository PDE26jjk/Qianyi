...
- [x] 2.4 Apply a selection to the 3D viewport by selecting exactly the meshes
  of the chosen panels and leaving every non-panel object alone
- [x] 2.5 Keep the last applied signatures and a busy flag as scene temp data so
  an apply is never mirrored back, and make a mesh whose panel is gone be
...
- [x] 3.1 Verify 3D to pattern editor: selecting one panel's mesh selects that
  panel and not another, and clearing the mesh selection clears the panels
- [x] 3.2 Verify pattern editor to 3D: selecting a panel selects its mesh and
  deselects the mesh of an unselected panel
...
- [ ] 3.4 Verify in a live session that a collider selected in 3D leaves the
  panel selection alone and stays selected itself
...
- [x] 4.1 Give the pattern renderer's outline call a line width, set explicitly
  on every call so the width cannot leak between panels
- [x] 4.2 Draw a second, thicker outline in a dedicated highlight colour for a
  selected panel, after its ordinary outline and its internal lines
- [ ] 4.3 Verify in a live session that the highlight is drawn in every display
  mode and disappears
  when the panel is deselected, and that the invalid-outline marker still shows
  on a selected panel
