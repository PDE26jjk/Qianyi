...
the pattern, the mesh and the simulation's own numbers in the same place.
...
- **THEN** the patterns are redrawn with their sampled mesh edges visible, and
  no pattern's outline, mesh, sewing or simulation state is modified
...
the sampled pattern mesh over the outline. `solid` SHALL fill the pattern with the
fabric's display color, which SHALL be a per-fabric property with a neutral
default. `debug` SHALL color the pattern mesh from the engine's per-vertex debug
colors. `stress` SHALL color it by a per-vertex strain the editor derives from
the pattern's rest and simulated vertex sets, on a documented ramp with stated
saturation values, because the engine does not report a per-vertex stress
today.

#### Scenario: Solid uses the fabric color

- **WHEN** two patterns use fabrics with different display colors and the mode is
  `solid`
- **THEN** each pattern is filled with its own fabric's color

#### Scenario: Stress without simulation data

- **WHEN** the mode is `stress` and no simulation data has been produced yet
- **THEN** the patterns are drawn as in `solid` and the pattern says that no
  simulation data is available

#### Scenario: Strain is what is drawn

- **WHEN** a pattern that has been stepped is shown in `stress`
...
Changing the display mode SHALL NOT resample a pattern, rebuild a mesh, change a
...
- **WHEN** the mode is switched several times on a project with generated
  patterns and sewings
- **THEN** the pattern meshes, the sewing pairings and the last simulated frame
  are unchanged

### Requirement: The 3D viewport has its own overlay pattern
...
- **THEN** the simulated patterns are drawn in the viewport with the strain ramp
...
- **THEN** the patterns are drawn with the engine's per-vertex debug values
...
the drawing without rebuilding anything and without touching the patterns.
...
- **THEN** the colouring stays on the cloth without flickering, and no batch is
  rebuilt and no pattern is modified

#### Scenario: The colouring still respects the scene

- **WHEN** a pattern is behind another pattern from the current viewpoint
- **THEN** the nearer pattern hides it, because the pass is depth tested
...
the patterns' current meshes - and MUST NOT require a simulation: it SHALL be
...
- **WHEN** the seam overlay is enabled on a freshly sewn project that has never
  been simulated
- **THEN** the seams are drawn between the paired stitch vertices of the patterns
  as they are placed now

#### Scenario: A shape key that is not the simulated one

- **WHEN** the patterns are showing a shape key other than the simulated one
...
- **WHEN** a seam is behind a pattern from the current viewpoint
- **THEN** the pattern hides it
...
The pattern window SHALL draw the fabric grain line rotated so that a pattern
...
uses for the default warp direction. Rotating a pattern's grain direction SHALL
...
- **WHEN** a pattern with the default grain direction shows its grain line
- **THEN** the drawn line runs vertically through the pattern in the pattern
  window

#### Scenario: Rotating the grain direction

- **WHEN** a pattern's grain direction is rotated by 90 degrees from the default
- **THEN** the drawn line follows and becomes horizontal, and the value the
  engine receives is the rotated one the pattern reports
