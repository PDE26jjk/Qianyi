"""A pattern whose own parameter changes its edge list: the hook's test case."""
...
COMPONENT_ID = "notched_pattern"
VERSION = 1
LABEL = "Notched Pattern"
CATEGORY = "Patterns"
...
def build(params: dict) -> list[PatternSpec]:
    ...
    if depth < MIN_NOTCH_DEPTH:
        ...
        return [PatternSpec(name="notched", vertices=vertices, edges=edges)]

    # A notch replaces the straight top edge with three edges, so the pattern's
