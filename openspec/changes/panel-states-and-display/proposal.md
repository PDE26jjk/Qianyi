...
designer says about a panel *beyond its outline*: that a panel is only a
...
those statements as panel state and as viewport display, and a pattern maker
who cannot find them reads the add-on as a simulator, not as a design tool.

## What Changes

- Add a per-panel simulation state: **simulate**, **excluded** (not solved and
  not a collider), **frozen** (held where it is, still a collider) and
  **stiffened** (solved with a stiffness multiplier on a per-panel fabric
...
- Add 3D viewport overlays for the same data: seam lines, stress/debug vertex
  ...
  panel instead of the current always-on debug registration.
- Add an avatar silhouette guide: project the collider meshes into the pattern
  window along a per-panel axis so a panel can be aligned against the body by
...
- `panel-simulation-states`: the per-panel simulate / excluded / frozen /
...
- `avatar-silhouette-guide`: projecting collider meshes into a panel's 2D space
  and drawing the resulting silhouette in the pattern window as an alignment
  reference.

### Modified Capabilities

- `agent-scripting-surface`: the documented data model gains the panel's
...
- Model: `Qianyi/model/pattern.py` (new state and display properties),
  `Qianyi/model/fabric.py` (per-panel override used by the stiffened state),
  `Qianyi/model/qianyi_project.py` (state propagation to copies and generators).
- Bridge: `Qianyi/simulation/simulation_data.py` /
  `Qianyi/simulation/simulation_manager.py` - excluded panels leave the mesh
  list, frozen panels arrive as fully pinned vertices, and only stiffened
  panels carry an overridden fabric block.
...
- UI: `Qianyi/ui/panels/patterns.py`, `Qianyi/ui/panels_3d/simulation_object.py`,
  a new display panel, and `Qianyi/declarations.py`.
...
- Engine: no engine change is required. The pin weights, the collider object
  ...
  per-panel fabric override, and the plasticity-based freeze is a separate
