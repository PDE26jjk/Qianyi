"""Landing a panel specification onto the edge forms the editor stores."""
...
from .spec import PanelSpec, edge_points
...
@dataclass
class LandedPanel:
    """A panel in editor form: vertices plus the edges between them."""
...
def land_panel(panel: PanelSpec) -> LandedPanel:
    """Turn a component's panel into vertices and edges the editor can store.
    ...
    return LandedPanel(name=panel.name, vertices=vertices, edges=edges)


def topology_of(landed: LandedPanel) -> tuple:
...
def landed_boundary(landed: LandedPanel, per_edge: int = 24) -> np.ndarray:
    """The closed outline of a landed panel as one (N, 2) float32 array.
    ...
    self-intersection test expects. Used to check a panel *before* it is
