...
- A pattern today is a 2D outline: a closed, counter-clockwise loop of edges over a
  ...
  primitive.
- Pattern meshes are sampled from the boundary at the pattern's granularity, with
...
- A sewing is stored as `(edge identity, normalized position)` for each side.
  The section list of a pattern is closed and linked across edges, so one side of
  a sewing can span consecutive edges.
- The simulation bridge consumes each pattern as an independent mesh payload with
  its own fabric, collision layer, grain direction and world matrix; it does not
  know how the pattern was created.
...
- A Blender-free component library that turns a parameter block into patterns,
  with deterministic geometry and deterministic edge labels.
- Generators that own those patterns inside a project and rebuild them
  automatically.
- Generated patterns that are ordinary patterns everywhere else, so they can be
  sewn to hand-drawn patterns and simulated through the existing path.
...
Components are ordinary Python functions that compute pattern geometry from
...
### D2 - Generators own ordinary patterns

A project gains a generator collection. A generator stores a component id and a
parameter block, and produces ordinary patterns that carry one extra field
pointing back at their generator. Sewing, mesh generation, the simulation
bridge and the UI keep working with a single pattern type.

*Alternatives*: a separate generated-pattern type stored in its own collection
(rejected: property-group collections are single-typed, so a subclass cannot be
stored in the existing pattern collection, and a second collection would force
every traversal - sewing, mesh rebuild, simulation setup, UI lists, project
save/load - to handle two sets); generator fields on every pattern (rejected: it
couples ordinary patterns to this feature).

### D3 - Sewings keep their current storage

A sewing stays `(edge identity, normalized position)` per side. Generated patterns
therefore need no new sewing representation and can be sewn to hand-drawn
patterns without any change to the sewing layer.
...
### D6 - No fixed pattern origin; stability instead
...
the pattern, because the rebuild relies on the new mesh staying close to the old
...
If a pattern's edge list is unchanged, the rebuild rewrites the existing edges in
...
After rewriting a component's patterns the system calls an optional hook that can
...
*Status*: validated by the prototype (the notched pattern, whose notch parameter
replaces one edge with three). The hook receives a plain-data context - the
previous edges and the rebuilt edges, each as `(pattern, index, uuid, label,
...
### D9 - One generator owns a group of patterns

Patterns are addressed by slot name inside a generator, and regeneration reuses
the pattern that already holds a slot, so a pattern keeps its identity when the
component changes. Deleting any pattern of the group deletes the group; detaching
converts the whole group into ordinary patterns and removes the generator.

*Alternatives*: one generator per pattern (rejected: a garment part is naturally
several patterns - a skirt, a bodice with sleeves - and the user would have to
assemble them and hand-stitch the internal seams); per-pattern detach or delete
(rejected: partial group states multiply the cases for little gain).

### D10 - Parameters apply immediately

A parameter change rebuilds the patterns at once; there is no apply step. The
rebuild is a pure function of the parameter block, so it is cheap and
repeatable.

### D11 - Locking

Generated patterns refuse geometry edits (vertices, spline points, edges,
...
copies, and deletion. Detached patterns are fully editable.
...
remapped together with their source pattern.
...
- Geometric matching can select the wrong edge on near-symmetric patterns -> labels
...
- Locking may miss an editing path (scripts, context menus) -> audit the
  ...
  alone.
- Deleting a group also deletes sewings that joined a generated pattern to a
  hand-drawn pattern -> this matches the existing delete behaviour; document it.
...
- No data migration: existing `.blend` files are untouched in format, and
  detached patterns are ordinary patterns.
- Rollback: remove the generator model, operators and UI; because generated
  patterns are ordinary patterns, detaching before rollback leaves the user's work
  intact.
...
- Whether an arc primitive is worth adding to the editor once generated patterns
  are in daily use.
