...
from ..extras.io import get_handler, get_extensions
...
class NODE_OT_qmyi_import(Operator, ImportHelper):
    ...
    def invoke(self, context, event):
        exts = get_extensions()
        console.info("exts", exts)
        self.filename_ext = ";".join(exts) + ";"
        self.filter_glob = ";".join(f"*{e}" for e in exts)
    ...
    def execute(self, context):
        ...
        for file in self.files:
            ...
            handler = get_handler(ext.lower())
            if not handler:
                console.error("Not support file type: ", ext)
                continue
            filepath = os.path.join(self.directory, file.name)
            console.info("Importing", filepath)
            handler.import_file(context, filepath, temp_dir)
            ...
            #             pattern: Pattern = tree.add_pattern()
            ...
            #                     if iLineType == 3:
            #                         control1, control2 = M[:2, :2].T @ parse_xy(lineListPoint[0]["v2Position"]), M[
            #                             :2, :2].T @ parse_xy(
