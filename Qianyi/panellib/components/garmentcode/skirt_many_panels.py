...
from ._panels import thin_skirt_spec
...
SCHEMA = {
    ...
    "params": {
        ...
        "n_panels": {"type": "int", "unit": "panels", "min": 4, "max": 15,
...
def build(params: dict):
    ...
    n_panels = max(1, int(round(float(params["n_panels"]))))
    ...
    panel_width = adjusted_waist / n_panels
    flare = 1.0 + suns * length * 2.0 * math.pi / adjusted_waist
    bottom_width = panel_width * flare

    # Deliberate comprehension: one PanelSpec per panel, a list of Python
