...
- [x] 2.2 Add the shared name resolution and refusal helpers (project, pattern, edge by index or label, fabric) and verify an unknown name is refused with the names that exist (`_address.py`; the probe checks an unknown pattern, an unknown edge label, an unknown fabric and an unknown component id)
...
- [x] 3.5 Implement the per-pattern sewing list on the shared edge-to-sewing index and verify it equals the project's sewings filtered by that pattern (one builder for both, so they cannot disagree; the probe compares the indexes)
...
- [x] 3.8 Implement the internal line edits and verify adding and removing one changes the pattern's mesh (258 -> 254 -> 258 vertices)
- [x] 3.9 Implement the instance chain rule for edits and verify a pattern with a mirror copy receives the same local geometry in both members and reports the chain (the probe edits the source and reads the same `p0` from the copy)
- [x] 3.10 Implement placement (anchor in millimetres, rotation, grain direction, collision layer, mirror in place) and verify the pattern moves and reports its new placement
...
- [x] 3.12 Implement `remove` and verify it reports the removed names and the sewings that went with them without asking, and that a pattern a generator owns is refused with a pointer to detach or to the generator's remove
...
- [x] 4.4 Implement `list`, the per-pattern list and `remove`, and verify the two lists agree and a removed seam disappears from both
...
- [x] 5.5 Implement `generators.create` and verify the patterns are built, named in the result, and that an unknown parameter is refused while a value outside its range is clamped and reported (99.0 comes back as 5.0, the schema's maximum)
- [x] 5.6 Implement `set_params` as one call with one rebuild and verify the report carries the in-place, rebuilt, created, removed, remapped, dropped and invalid counts (`in_place: 2` for a two-pattern generator)
- [x] 5.7 Implement the carried-over simulation flag in the rebuild report and verify the two cases: a rebuild after a run reports carried positions and leaves the patterns off their rest pose, a rebuild before any run reports nothing to carry (`untouched` / `simulation_carried: false` before a run; `simulated` / `true` after one, with the seams intact)
- [x] 5.8 Implement `detach` and `remove` and verify detach keeps the patterns as ordinary ones and remove takes the group and its sewings with it
- [x] 5.9 Verify a generated pattern is readable and editable through the surface and reports that it comes from a generator, while the interactive lock still refuses the same edit through an operator (the surface half is verified; the operator lock is the other change's guard and its click-through needs a viewport)
...
- [x] 6.4 State the units and the coordinate spaces for the new calls (millimetres for pattern space and anchors, metres for the mesh and the world), the naming and suffix rule, the instance chain rule, the undo granularity, and the fact that a rebuild can rewrite an edited generated pattern (a `help("patterns")` topic and a section of the reference file)
