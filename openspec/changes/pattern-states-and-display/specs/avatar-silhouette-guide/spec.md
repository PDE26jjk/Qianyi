...
Shows the body behind a pattern in the pattern window, so a pattern maker can
align a pattern to the avatar by eye - outline, surface and seams - at the same
...
millimetres of pattern space and a pattern can be aligned against the body by eye.
...
- **THEN** the silhouette moves by exactly that much and no pattern data changes

#### Scenario: Changing the projection axis

- **WHEN** the projection axis is changed from front to side
- **THEN** the silhouette is redrawn for the new axis and no pattern data changes
...
change, SHALL never contribute to a pattern's vertices, mesh or the engine
payload, and SHALL NOT require any pattern to be regenerated when it changes.

#### Scenario: Moving the avatar

- **WHEN** a selected object is moved or rotated while the guide is on
- **THEN** the silhouette follows in the next redraw and the patterns' vertices,
...
- **THEN** the silhouette follows the deformation without any pattern being
