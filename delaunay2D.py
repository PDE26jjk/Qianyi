...
class Delaunay2D:
    ...
    def circumcenter_2(self, tri):
        """
        ??????????????
        :param p1, p2, p3: ?????????? (x, y)
        :return: (????, ?????)
        """
        p1, p2, p3 = np.asarray([self.coords[v] for v in tri])
        # ????
        x1, y1 = p1
        x2, y2 = p2
        x3, y3 = p3
        ...
        # ??????
        cx = (C1 * B2 - C2 * B1) / denominator
        cy = (A1 * C2 - A2 * C1) / denominator

        # ??????????????
        radius = (cx - x1) ** 2 + (cy - y1) ** 2
    ...
    def addPoint(self, p):
        ...
        for T in bad_triangles:
            for tri_op in self.triangles[T]:
                if tri_op not in bad_triangles and self.inCircleFast(tri_op,p):
