# agent-outline-crossing-policy Specification
...
An answer that produced a crossing outline SHALL report it: the pattern, its
validity, and the crossing point when one is known. When the mesh stage therefore
did not rebuild the pattern's mesh, the answer SHALL say the mesh is stale and
...
#### Scenario: Reading a pattern that already crosses

- **WHEN** a pattern whose outline crosses is read
...
A crossing outline MUST NOT be handed to the mesh sampler: the pattern keeps the
mesh it had, and the answer says so. A simulation MUST NOT start while any
participating pattern's outline crosses.

#### Scenario: No mesh for a crossing outline

- **WHEN** a crossing outline is stored with the flag on
- **THEN** the pattern's mesh is the one it had, and the answer reports that it was not rebuilt

#### Scenario: A simulation is refused

- **WHEN** a simulation is prepared while a participating pattern crosses
- **THEN** it is refused with the offending patterns named
...
those patterns. A rebuild MUST NOT gain a refusal for crossing, and MUST NOT take
...
- **WHEN** a parameter change leaves a pattern's outline crossing
- **THEN** the rebuild is applied, that pattern keeps its previous mesh, and the report counts it

#### Scenario: The report names the patterns

- **WHEN** a rebuild leaves patterns crossing or degenerate
...
the answers SHALL stop reporting the pattern as crossing or its mesh as stale.
...
- **THEN** the mesh is rebuilt and the next read reports the pattern as valid

#### Scenario: The read-only check still reports

- **WHEN** the outline check is called as a read
- **THEN** it reports the crossing patterns and their crossing points without raising, whatever was allowed before
