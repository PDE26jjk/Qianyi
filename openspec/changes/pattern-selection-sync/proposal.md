...
A pattern exists twice in the add-on: as a mesh object in the 3D viewport and as a
pattern in the pattern editor. Selecting one currently says nothing to the
other, so a pattern maker who picks a pattern on the body has to find it again by
name in the editor, and a selection made in the editor is invisible on the body.
On top of that, the pattern window outlines every pattern in the same colour, so
...
- With it on, the two selections mirror each other:
  - selecting a pattern's mesh in the 3D viewport selects that pattern in the
    pattern editor, together with the other members of its instance chain, and
    deselects the patterns that are no longer selected;
  - selecting patterns in the pattern editor selects their meshes in the 3D
...
- Only pattern meshes take part. Colliders, the body and any other object keep
  their selection, and a pattern with no mesh has nothing to select in 3D.
- Give a selected pattern a **highlighted outline** in the pattern window: a
  second, thicker outline in a highlight colour, drawn over the ordinary one, in
  every display mode. Today the outline is the same white for every pattern, so a
...
- `pattern-selection-sync`: the header toggle, the two-way mirroring of the 3D
...
(none - no existing specification covers the selection of patterns)
...
- Selection model: `Qianyi/model/qianyi_project.py` (`selected_patterns` and the
  ...
  `Qianyi/model/generator.py` (`instance_chain` decides what "the pattern" means).
