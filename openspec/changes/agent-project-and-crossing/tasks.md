...
- [x] 1.1 Implement `projects.list()` and verify it reports every project with the name it is addressed by, whether it is active, and its panel, sewing, fabric and generator counts (an empty scene reports `{"projects": [], "active": None}`; the test scene reports its project with its counts)
- [x] 1.2 Implement `projects.create(name=None, activate=True)` - the node tree, the identities of the project and its default fabric, both names and the active index - and verify that a panel created straight afterwards needs no further setup in a scene that started empty (the blank section creates a project and then a panel with its mesh, in one session)
...
- [x] 1.6 Implement `projects.remove(name)` and verify it reports the panels and sewings that went with it (and, as the notes below record, that the panels' mesh objects go with them)
- [x] 1.7 Verify the empty-scene path: `projects.list()` returns nothing, a panel call before any project is refused with the call that makes one, and a project created after that makes the same panel call succeed
- [x] 1.8 Verify the identity work is what makes it work: create a project with the node tree call alone and confirm the panel call still fails, so a future change cannot quietly drop the identity step (the blank section keeps this check: `node_tree_alone_error: AssertionError`)

## 2. The crossing policy

- [x] 2.1 Add `allow_crossing=False` to `create`, `set_point`, `add_point` and `remove_point` and verify the default refuses a crossing outline before anything is written (`moving that point would cross itself near (0.000, 33.333)`, and the panel keeps its validity)
...
- [x] 2.6 Verify a crossing outline is never handed to the mesh sampler: with the flag on, the panel keeps the mesh it had (166 vertices before and after) and the answer says it is stale; a panel created crossing has no mesh object at all
- [x] 2.7 Verify a simulation still refuses a crossing panel by name, and that `validate()` still reports it as a read without raising
...
- [x] 2.9 Add the panel names and the stale-mesh flag to the generator rebuild report, and verify a parameter change that leaves a panel crossing is still applied, still counted, and now names the affected panels (`notch_depth` 10 -> 200 on a 100 mm panel: `invalid_panels: 1`, `invalid_panel_names: ["notched"]`, `stale_meshes: ["notched"]`)
- [x] 2.10 Verify a generator rebuild takes no crossing argument and gains no refusal: set a parameter that leaves a panel crossing and confirm the call succeeds exactly as it did before this change (`takes_flag: False`, the rebuild applies and reports)
...
- [x] 4.1 Extend `tools/probe_agent_api.py` with a blank-scene section (no scene file): create a project, build a panel, list, rename, activate a second project, remove one - and verify zero failures (`--blank`: 13 checks, 0 failures)
...
- `projects.remove` has to take the panels out through the model layer before the
  node tree goes. A panel's mesh object is a scene object, not part of the tree,
  ...
  through `remove_patterns` deletes those meshes, as the ordinary panel removal
  already did.
- An ambiguous panel name is now refused. A generator names its panels after the
  component's slots, so a slot can collide with a hand-made panel: the probe hit
  exactly that (a hand-made `square` next to a `square` slot) and the surface
  answered with the first match. `pattern_or_refuse` now reports how many panels
